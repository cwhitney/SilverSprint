...
void SilverSprintApp::loadSettings()
{
	...
	if(fs::exists(targetPath)){
		try {
			...
			CI_LOG_I("Successfully loaded settings from " << targetPath);
			...
			Model::instance().setRollerDiameterMm(  config().get("settings", "roller_diameter_mm", 114.3f));
			Model::instance().setNumRacers(         config().get("settings", "num_racers", 2));
			...
			Model::instance().setRaceLengthMeters(  config().get("settings", "race_length_meters", 100.0f));
		}catch (exception &exc){
			CI_LOG_E("Error reading settings file: " << exc.what());
		}
...
void SilverSprintApp::draw()
{
	...
	gl::drawString( StateManager::instance().getCurrentAppStateString(), vec2(10, getWindowHeight() - 40) );
	gl::drawString( StateManager::instance().getCurrentRaceStateString(), vec2(10, getWindowHeight() - 20) );
#endif
}
