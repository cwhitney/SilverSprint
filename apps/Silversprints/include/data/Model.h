...
//                StateManager::instance().signalSerialDeviceSelected.emit(mSelectedPortName);
		...
		struct SerialDeviceDesc {
			...
			// Equality operator for comparing two SerialDeviceDesc objects
			bool operator==(const SerialDeviceDesc& other) const {
				return portName == other.portName && portDescription == other.portDescription;
			}
		};
